const HOME = Deno.env.get("HOME") ?? "";

export const CONFIG_DIR = `${HOME}/.config/brick`;
export const GLOBAL_CONFIG_PATH = `${CONFIG_DIR}/config.toml`;
export const REPO_CONFIG_NAME = ".brick.toml";

/** Legacy file names for migration detection */
export const LEGACY_GLOBAL_CONFIG_PATH = `${CONFIG_DIR}/config.yml`;
export const LEGACY_REPO_CONFIG_NAME = ".brick.yml";
export const REMOTE_SCHEMA_URL =
    "https://raw.githubusercontent.com/nikbrunner/brick/main/schema.json";
